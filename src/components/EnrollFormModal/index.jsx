...
const EnrollFormModal = () => {
    ...
    const handleSignupSubmit = (event) => {
        ...
        fetch(
            `https://generationsapi.herokuapp.com/api/auth/local/register`,
            {
              method: "POST",
              headers: {
    ...
    const handleLoginSubmit = (event) => {
        ...
        fetch(
            `https://generationsapi.herokuapp.com/api/auth/local`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
            ...
            .then((data) => {
                if(data.data === null){
                    throw new Error(`${data.error.message === "Invalid identifier or password" ? "Invalid username or password" : data.error.message}. Please try again.`);
                }
                sessionStorage.setItem("userSession", data.jwt);
                event.target.reset();
                setError(false);
                ...
                <div className="modal modal-center m:modal-middle">
                    <div className="modal-box w-10/12 md:w-6/12 max-w-4xl">
                        ...
                        <div className="text-center mb-6 text-gray-500 text-[16px]">
                            <small>Already have an account? <span className="link" onClick={() => setStatus("login")}>Login here</span></small> 
                        </div>
                        <div className={ error ? "bg-red-100 border border-red-400 text-red-700 px-4 py-2 rounded relative mb-6 text-xs" : "hidden bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-6 text-xs" } role="alert">
                            <span className="block sm:inline">{ message }</span>
                        </div>
                        <form name="enquiry" method="post" onSubmit={(event) => handleSignupSubmit(event)}>
            ...
            <input type="checkbox" id="enroll" className="modal-toggle" />
                <div className="modal modal-center m:modal-middle">
                    <div className="modal-box w-10/12 md:w-6/12 max-w-4xl">
                        ...
                        <div className="text-center mb-6 text-gray-500 text-[16px]">
                            <small>Don't have an account? <span className="link" onClick={() => setStatus("signup")}>Enroll here</span></small> 
                        </div>
                        <div className={ error ? "bg-red-100 border border-red-400 text-red-700 px-4 py-2 rounded relative mb-6 text-xs" : "hidden bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-6 text-xs" } role="alert">
                            <span className="block sm:inline">{ message }</span>
                        </div>
                        <form name="enquiry" method="post" onSubmit={(event) => handleLoginSubmit(event)}>
                            <input type="hidden" name="form-name" value="enquiry" />
                            <input
                                type="text"
                                name="identifier"
